...
int main()
{
    ...
    {
        if (buff_uart1.cnt > 0) {
            USART2_Send_byte(Rbuff_read(&buff_uart1));
        }
        if (buff_uart2.cnt > 0) {
            char_buff = Rbuff_read(&buff_uart2);
            if (char_buff == 'Z') {
                if (GPIOC->ODR) GPIOC->BSRR = GPIO_BSRR_BR13;
                else GPIOC->BSRR = GPIO_BSRR_BS13;
            }
            else USART1_Send_byte(char_buff);
        }
    }
}
